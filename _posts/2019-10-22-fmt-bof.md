...
Using `vmmap` we get to know the address of executable and writable libc and using the address we leaked from `%3$lx` input we have the leaked libc address, we just subtracted the leak address from the `libc-2.27.so` address, we get the offset.
